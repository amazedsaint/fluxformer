...
def compare_mean_vs_perhead(T=16,H=4,slopes=None):
    # Generate per-head bias slopes if not provided; ensure length matches H
    if slopes is None:
        # Use a simple range of slopes that encourage specialization across heads
        slopes=np.linspace(0.005, 0.04, num=H, dtype=float)
